...
describe('extractAncestry — base slice (Goblin fixture)', () => {
  ...
  it('extracts mechanics.hit_points from h2 section', async () => {
    const html = await loadFixture('ancestry-goblin.html');
    const out  = await parseAonHtml(html, 'https://2e.aonprd.com/Ancestries.aspx?ID=4');
    const m = (out as unknown as { mechanics: { hit_points: number | null } }).mechanics;
    assert.equal(m.hit_points, 6);
  });

  it('extracts mechanics.size = Small', async () => {
    const html = await loadFixture('ancestry-goblin.html');
    const out  = await parseAonHtml(html, 'https://2e.aonprd.com/Ancestries.aspx?ID=4');
    const m = (out as unknown as { mechanics: { size: string | null } }).mechanics;
  ...
  it('extracts mechanics.speed = 25', async () => {
    const html = await loadFixture('ancestry-goblin.html');
    const out  = await parseAonHtml(html, 'https://2e.aonprd.com/Ancestries.aspx?ID=4');
    const m = (out as unknown as { mechanics: { speed: number | null } }).mechanics;
  ...
  it('extracts mechanics.attribute_boosts as list with Dexterity', async () => {
    const html = await loadFixture('ancestry-goblin.html');
    const out  = await parseAonHtml(html, 'https://2e.aonprd.com/Ancestries.aspx?ID=4');
    const m = (out as unknown as { mechanics: { attribute_boosts: string[] } }).mechanics;
    assert.ok(m.attribute_boosts.includes('Dexterity'), 'Goblin should have Dexterity boost');
  });

  it('extracts mechanics.attribute_flaws non-empty', async () => {
    const html = await loadFixture('ancestry-goblin.html');
    const out  = await parseAonHtml(html, 'https://2e.aonprd.com/Ancestries.aspx?ID=4');
    const m = (out as unknown as { mechanics: { attribute_flaws: string[] } }).mechanics;
    assert.ok(m.attribute_flaws.length > 0, 'Goblin should have attribute flaw');
  });

  it('parses popular_edicts and popular_anathema', async () => {
    const html = await loadFixture('ancestry-goblin.html');
    const out  = await parseAonHtml(html, 'https://2e.aonprd.com/Ancestries.aspx?ID=4');
    const r = out as unknown as { popular_edicts: string | null; popular_anathema: string | null };
    assert.ok(r.popular_edicts !== null && r.popular_edicts.length > 0, 'should have popular edicts');
    assert.ok(r.popular_anathema !== null && r.popular_anathema.length > 0, 'should have popular anathema');
  });
});
...
describe('extractAncestry — features slice', () => {
  it('captures free-form h2 sections (You Might…, Beliefs, Names)', async () => {
    ...
    const hasNarrative = names.some((n) => /You Might|Beliefs|Names|Society|Physical/i.test(n));
...
describe('extractAncestry — raw_fields strip', () => {
  it('does not contain claimed mechanic labels (Hit Points, Size, Speed)', async () => {
    ...
    for (const claimed of ['Hit Points', 'Size', 'Speed', 'Languages']) {
      assert.ok(!(claimed in rf), `${claimed} should be stripped from raw_fields`);
    }
  });
});
