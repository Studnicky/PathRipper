// Unit tests for equipment concept capability nodes.
...
async function primeAndRunFull(fixtureName: string, url: string) {
  ...
  await equipmentBaseNode.execute(Batch.of(state), stubContext);
  await equipmentMechanicsNode.execute(Batch.of(state), stubContext);
  await finalizeEquipmentNode.execute(Batch.of(state), stubContext);
  return ParsedOutput.as<EquipmentOutput>(state.output);
}
...
describe('extract:equipment-base — equipment-adventurers-pack', () => {
  it('produces _type, name, equipment_id', async () => {
    ...
    const result = await equipmentBaseNode.execute(Batch.of(state), stubContext);
    assert.ok(result.has('success'));

    const out = ParsedOutput.as<EquipmentOutput>(state.output);
...
describe('extract:equipment-mechanics — equipment-adventurers-pack', () => {
  it('produces price, bulk, activations', async () => {
    const state = await primeState('equipment-adventurers-pack.html', 'https://2e.aonprd.com/Equipment.aspx?ID=1');
    await equipmentBaseNode.execute(Batch.of(state), stubContext);
    const result = await equipmentMechanicsNode.execute(Batch.of(state), stubContext);
    assert.ok(result.has('success'));
...
describe('finalize:equipment — equipment-adventurers-pack', () => {
  it('produces raw_fields, variants, pfs_note', async () => {
    ...
    await equipmentBaseNode.execute(Batch.of(state), stubContext);
    await equipmentMechanicsNode.execute(Batch.of(state), stubContext);
    const result = await finalizeEquipmentNode.execute(Batch.of(state), stubContext);
    assert.ok(result.has('success'));
