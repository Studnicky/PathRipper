...
const makeContext = (services: Partial<RipperServices>): NodeContextInterface<RipperServices> => ({
  services: {
...
describe('builtinNodes', () => {
  ...
  describe('exports', () => {
    it('every built-in node has a name and outputs array', () => {
      ...
      for (const n of nodes) {
        assert.ok(typeof n.name === 'string' && n.name.length > 0, `${n.name}: missing name`);
        assert.ok(Array.isArray(n.outputs) && n.outputs.length > 0, `${n.name}: missing outputs`);
      }
    });
  });
  ...
  describe('HtmlFetchNode', () => {
    it('fetches via services.htmlScraper and stores html + resolved url on the page', async () => {
      ...
      const result = await HtmlFetchNode.execute(state, ctx);
      ...
      assert.ok(['success', 'cached'].includes(result.output));
    });
    ...
    it('sets _raw on state.page by default (includeRawContent absent)', async () => {
      ...
      await HtmlFetchNode.execute(state, ctx);
  ...
  describe('HtmlWriteRawNode', () => {
    it('writes html to <outDir>/<target>/raw/<url-slug>.html', async () => {
      ...
      const ctx   = makeContext({ target: { id: TARGET, cfg: {} }, outDir });
      await HtmlWriteRawNode.execute(state, ctx);
    ...
    it('throws when html is missing', async () => {
      ...
      await assert.rejects(
        HtmlWriteRawNode.execute(state, ctx),
        (e: unknown) => e instanceof ExternalSchemaError,
      );
    });
  });
  ...
  describe('JsonWriteNode', () => {
    it('writes JSON to <target>/<slug>.json with no pluginTaskName', async () => {
      ...
      const ctx   = makeContext({ target: { id: TARGET, cfg: {} }, outDir });
      const r     = await JsonWriteNode.execute(state, ctx);
  ...
  describe('JsonlAppendNode', () => {
    it('appends one JSON line per invocation to all.jsonl', async () => {
      const ctx = makeContext({ target: { id: TARGET, cfg: {} }, outDir });
      await JsonlAppendNode.execute(buildState({ output: { n: 1 } }), ctx);
      await JsonlAppendNode.execute(buildState({ output: { n: 2 } }), ctx);
      await JsonlAppendNode.execute(buildState({ output: { n: 3 } }), ctx);
    ...
    it('does NOT include _raw in appended JSONL rows', async () => {
      ...
      await JsonlAppendNode.execute(state, ctx);
  ...
  describe('WikiWriteRawNode', () => {
    it('writes wikitext to <outDir>/<target>/raw/<slug>.txt', async () => {
      ...
      const ctx   = makeContext({ target: { id: TARGET, cfg: {} }, outDir });
      await WikiWriteRawNode.execute(state, ctx);
  ...
  describe('ValidateSchemaNode', () => {
    it('returns valid when config.outputSchema is unset', async () => {
      ...
      const ctx   = makeContext({ target: { id: TARGET, cfg: {} }, outDir });
      const r     = await ValidateSchemaNode.execute(state, ctx);
