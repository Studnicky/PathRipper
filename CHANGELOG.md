...
- v0.x implementation complete (369 unit tests + 15 integration tests, all green):
  - **RDF wrapper layer** (`src/rdf/*`): Formats, DataFactory, TermGuards, Namespaces, IRIUtils, BaseIRIResolver, Vocab (RDF/RDFS/OWL/XSD/SHACL + STANDARD_PREFIXES), Dataset, Parser (n3+jsonld dispatcher), Serializer (n3.Writer+jsonld.fromRDF dispatcher), Canonicalize (rdf-canonize RDFC-1.0), SyntaxValidator (parse round-trip), GraphBuilder (vendored from semantics/rdf-builder, trimmed for v0.x). Application code never imports `n3`/`jsonld`/`rdf-canonize`/`@rdfjs/*` directly — enforced by ESLint `no-restricted-imports` rule.
  - **SHACL wrapper** (`src/shacl/ShaclGate.ts`): wraps `rdf-validate-shacl` with `formatReport` for human output; uses the validator's bundled defaultEnv (no factory option — empirically required, plan 13 corrected).
  - **Output layer** (`src/output/*`): `OutputInterface`, `OutputReport`, `FormatResolver`, `FileOutput` with atomic-write (tmp+fsync+rename), SHACL pre-write hook with `validation.report.{txt,ttl}` quarantine emission, graph-collapse via `output.graph`, dryRun mode.
  - **Quarantine writer** (`src/quarantine/QuarantineWriter.ts`): four buckets (unknown/conflicts/projection/output) with SHA-1 record IDs, `summary()`, exit-code helper.
  - **Config + schemas** (`src/config/*`, `src/schemas/*`): AJV-validated `SquashageConfig.loadFromFile`, JSON Schema for `output`/`target`/root with `OutputConfigInterface` derived via `json-schema-to-ts`.
  - **Built-in tasks** (`src/tasks/*`): `json:read` (file/JSONL reader with quarantine on parse failure), `rdfjs:finalize` (orchestrator-invoked finalize that drives FileOutput), `index.ts` (side-effect entry registering both).
  - **Orchestrator** (`src/orchestrators/SquashageOrchestrator.ts`): builds run-wide `PipelineContextInterface`, walks input directory recursively, drives `ConcurrentPipeline.executeAll`, strips `rdfjs:finalize` from the per-record queue and invokes it once after the final batch settles, returns `RunResultInterface`.
  - **CLI** (`src/cli/cli.ts`): `build`/`classify`/`inspect` subcommands wired to the orchestrator with `--out`, `--format`, `--in`, `--dry-run` overrides; `buildCli()` factory + ESM `isMain` guard for test friendliness.
  - **Integration test** (`tests/integration/build-trig.test.ts`): full pipeline smoke covering `json:read → fixture:classify → fixture:squash → rdfjs:finalize`, malformed-record quarantine, and SHACL pass/fail cases.

- Deterministic classifier cascade — six idiomatic task classes the user opts into via `targets[].pipeline`, each instantiated per-target with its frozen, AJV-validated config at run startup. Built on a per-run `TaskRegistry` so two targets with conflicting rule configs in the same run never collide. 557 unit tests + 22 integration tests cover the cascade.
  - **`classify:source`** (`SourceClassifier`): emits a `__source__` marker proposal from the record's `_source` block.
  - **`classify:structural`** (`StructuralClassifier`): predicate-based structural gate (required keys, literal discriminators).
...
- **Closed-vocabulary `Predicate` engine** (`src/classification/predicates/`): purely deterministic predicate language used by Structural and Rules classifiers. Closed operator set — `equals`, `notEquals`, `in`, `notIn`, `exists`, `missing`, `type`, `regex` (must be anchored), `length`, `range`, plus `all`/`any`/`not` composition. Paths are RFC 6901 JSON Pointers. Compiled at startup (RegExp pre-built, path segments pre-split, AST tagged-union); runtime evaluation does no allocation. AJV schema at `src/schemas/predicate.schema.json` validates raw config against the closed vocab.
- **`ClassificationFactory.build`** — single static method that consumes the target's `classification` config block, compiles raw predicates via `Predicate.compile`, reads + AJV-compiles schema files, instantiates the six classifier task instances, and returns them keyed by task name. All file I/O happens here at startup, never per-record.
- **AJV cross-validation** in `SquashageConfig`: walks each target's `pipeline:` and asserts the matching `classification.<key>` config sub-block exists and is non-empty for every classify task listed. Enforces that `classify:conflict` is required when ≥2 distinct class-proposing classifiers (`structural`, `rules`, `schema`) are listed. Misconfigured targets fail at config-load with `SquashageConfigError`, not at first record.
- **Per-run `TaskRegistry`**: `Pipeline` constructor now accepts an optional `registry?: TaskRegistry` argument. The static surface (`TaskRegistry.register`/`get`/...) is preserved as a back-compat delegating wrapper around a module-private default instance; built-ins (`json:read`, `rdfjs:finalize`) self-register on the default at module load. The orchestrator constructs a fresh `TaskRegistry` per run, seeds built-ins from the default, registers per-target classifier instances on it, and threads the instance into `Pipeline`.
- **Plan 13 "Deterministic Classifier Menu"** section: documents the six task names, their required config blocks, the closed predicate vocabulary, the per-target registry isolation, and the determinism guarantees (no `Math.random`, `Date.now`, network, or fs after startup; tiebreaks are deterministic across runs and machines).
- v0.x implementation complete (369 unit tests + 15 integration tests, all green):
  - **RDF wrapper layer** (`src/rdf/*`): Formats, DataFactory, TermGuards, Namespaces, IRIUtils, BaseIRIResolver, Vocab (RDF/RDFS/OWL/XSD/SHACL + STANDARD_PREFIXES), Dataset, Parser (n3+jsonld dispatcher), Serializer (n3.Writer+jsonld.fromRDF dispatcher), Canonicalize (rdf-canonize RDFC-1.0), SyntaxValidator (parse round-trip), GraphBuilder (vendored from semantics/rdf-builder, trimmed for v0.x). Application code never imports `n3`/`jsonld`/`rdf-canonize`/`@rdfjs/*` directly — enforced by ESLint `no-restricted-imports` rule.
  - **SHACL wrapper** (`src/shacl/ShaclGate.ts`): wraps `rdf-validate-shacl` with `formatReport` for human output; uses the validator's bundled defaultEnv (no factory option — empirically required, plan 13 corrected).
  - **Output layer** (`src/output/*`): `OutputInterface`, `OutputReport`, `FormatResolver`, `FileOutput` with atomic-write (tmp+fsync+rename), SHACL pre-write hook with `validation.report.{txt,ttl}` quarantine emission, graph-collapse via `output.graph`, dryRun mode.
...
- Scraper layer wholesale: `src/scrapers/`, `src/crawlers/`, `src/orchestrators/ScrapeOrchestrator.ts`, `src/modules/cache/`, `src/modules/http/`, related types and tests, all `docs/*.html`, root-level `scrapers/*.js`, `errors/*.js` stray compiled artifacts. Orphan deps dropped: `bottleneck`, `cheerio`, `domhandler`, `wtf_wikipedia`.
- Scraper layer wholesale: `src/scrapers/`, `src/crawlers/`, `src/orchestrators/ScrapeOrchestrator.ts`, `src/modules/cache/`, `src/modules/http/`, related types and tests, all `docs/*.html`, root-level `scrapers/*.js`, `errors/*.js` stray compiled artifacts. Orphan deps dropped: `bottleneck`, `cheerio`, `domhandler`, `wtf_wikipedia`.
- Scraper layer wholesale: `src/scrapers/`, `src/crawlers/`, `src/orchestrators/ScrapeOrchestrator.ts`, `src/modules/cache/`, `src/modules/http/`, related types and tests, all `docs/*.html`, root-level `scrapers/*.js`, `errors/*.js` stray compiled artifacts. Orphan deps dropped: `bottleneck`, `cheerio`, `domhandler`, `wtf_wikipedia`.
- Scraper layer wholesale: `src/scrapers/`, `src/crawlers/`, `src/orchestrators/ScrapeOrchestrator.ts`, `src/modules/cache/`, `src/modules/http/`, related types and tests, all `docs/*.html`, root-level `scrapers/*.js`, `errors/*.js` stray compiled artifacts. Orphan deps dropped: `bottleneck`, `cheerio`, `domhandler`, `wtf_wikipedia`.
