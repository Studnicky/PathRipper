...
The Squashage v0.x branch — graph reconstitution pipeline that consumes Ripperoni JSON, classifies each record through a deterministic cascade, projects matched records into RDF/JS quads, and emits a single serialized RDF file (or interactive HTML graph). 662 unit tests + 22 integration tests + 43 e2e tests, all gates green. Branch ready for the v0.x npm release.

### Added

**Project bootstrap**
- Squashage workspace bootstrapped as a literal file copy of Ripperoni; package identity changed; config examples renamed to `squashage.config*.example.json`.
...
**Documentation**
- Plan 13 (`docs/plans/13-file-output-and-semantics-integration.md`) — full implementation record: orchestrator-driven `rdfjs:finalize`, AJV schemas, code standards inherited from Ripperoni verbatim, deterministic classifier menu, file inventory with importer-evidence-based deletion plan.
...
- Code standards inherited from Ripperoni verbatim (lint, tsc, AJV, hooks, CI, conventional commits, changelog gate, TSDoc density, logger discipline, module conventions).
