...
export class CosmosGraphRenderer {
  ...
  static #html(title: string, indexUrl: string): string {
    ...
    return [
      ...
      '      <button class="sq-physics-pause" id="sq-physics-pause">⏸ Pause</button>',
      '    </div>',
      '  </div>',
...
.sq-sidebar { width: 320px; flex: 0 0 320px; overflow-y: auto; overflow-x: hidden; background: #111111; border-left: 1px solid #2a2a2a; display: flex; flex-direction: column; scrollbar-width: thin; scrollbar-color: #333 transparent; }
.sq-sidebar::-webkit-scrollbar { width: 5px; }
.sq-sidebar::-webkit-scrollbar-track { background: transparent; }
.sq-sidebar::-webkit-scrollbar-thumb { background: #333; border-radius: 3px; }
...
#sq-details-body { font-size: 12px; word-break: break-word; overflow-wrap: break-word; overflow-x: hidden; }
.sq-detail-id { color: #e94560; font-weight: 600; margin-bottom: 4px; font-size: 14px; }
.sq-detail-iri { font-family: ui-monospace, monospace; font-size: 10px; color: #707070; margin-bottom: 6px; word-break: break-word; overflow-wrap: break-word; }
...
.sq-physics-pause { width: 100%; margin-top: 4px; background: rgba(192,159,239,0.12); border: 1px solid #c09fef; color: #c09fef; border-radius: 4px; padding: 4px 0; font-size: 11px; cursor: pointer; }
.sq-physics-pause:hover { background: rgba(192,159,239,0.3); }
.sq-physics-pause.sq-paused { background: rgba(129,199,132,0.12); border-color: #81c784; color: #81c784; }
.sq-physics-pause.sq-paused:hover { background: rgba(129,199,132,0.3); }
...
/* Properties table */
.sq-props-table { width: 100%; border-collapse: collapse; margin-bottom: 8px; font-size: 11px; }
.sq-props-table td { padding: 2px 4px; vertical-align: top; }
.sq-prop-key { color: #c09fef; white-space: nowrap; font-weight: 600; padding-right: 8px; width: 40%; }
.sq-prop-val { color: #c0c0c0; word-break: break-word; }
.sq-prop-list { margin: 0; padding: 0; list-style: none; }
.sq-prop-list li { display: inline; }
.sq-prop-list li:not(:last-child)::after { content: ", "; }
`;
  }
...
(function () {
  ...
  var manifest     = null;
  var frameMeta    = [];   // per-frame { label, color, nodeCount, nodeBase, dataFile }
  var queueItems   = [];
  ...
  var simPaused    = false;   // physics Pause/Resume toggle — gates the keep-alive re-energizer
  ...
  // ── Data shard cache: dataFile path → { props, nodeBase } | null ─────────
  var nodeDataCache = new Map();
  ...
  // Load a data shard (.json.gz) for the given dataFile path, store in nodeDataCache,
  // and call callback() when done (success or failure).
  function loadDataShard(cacheKey, dataFile, callback) {
    // Sentinel: set to null immediately to prevent double-fetch
    nodeDataCache.set(cacheKey, null);
    var shardUrl = new URL(dataFile, baseUrl).href;
    fetch(shardUrl)
      .then(function (r) {
        if (!r.ok) throw new Error('shard ' + r.status);
        if (typeof DecompressionStream === 'undefined' || r.body === null) throw new Error('no-decomp');
      ...
      .catch(function (err) {
        console.warn('[squashage] data shard load failed:', String(err && err.message || err));
        nodeDataCache.set(cacheKey, { props: {} });
        callback();
      });
  }
  ...
  function showNodeDetails(idx) {
    ...
    // Properties table — from lazy-loaded data shard
    var dataFile  = meta ? (meta.dataFile || null) : null;
    var cacheKey  = dataFile || '';
    var shardData = dataFile ? nodeDataCache.get(cacheKey) : undefined;

    if (dataFile && shardData === undefined) {
      // Not yet loaded — trigger fetch, show placeholder
      html += '<div class="sq-hint">Loading properties…</div>';
      loadDataShard(cacheKey, dataFile, function () {
        // Re-render once loaded (only if this node is still selected)
        if (selFsm.state === 'nodeSelected' && selFsm.selectedIdx === idx) {
          showNodeDetails(idx);
        }
      });
    } else if (shardData && shardData.props) {
      var nodeProps = shardData.props[String(idx)];
      if (nodeProps && Object.keys(nodeProps).length > 0) {
        html += '<table class="sq-props-table">';
        var keys = Object.keys(nodeProps).sort();
        for (var ki = 0; ki < keys.length; ki++) {
          var k = keys[ki];
          var vals = nodeProps[k] || [];
          if (vals.length === 0) continue;
          // Strip IRI prefix if present (key may be full IRI or local name).
          var localKey = k;
          var lastHash  = k.lastIndexOf('#');
          var lastSlash = k.lastIndexOf('/');
          var lastColon = k.lastIndexOf(':');
          var bestSep   = Math.max(lastHash, lastSlash, lastColon);
          if (bestSep >= 0 && bestSep < k.length - 1) localKey = k.slice(bestSep + 1);
          var displayKey = localKey.replace(/_/g, ' ').replace(/\\b\\w/g, function (c) { return c.toUpperCase(); });
          var displayVal = vals.length === 1
            ? esc(vals[0])
            : '<ul class="sq-prop-list">' + vals.map(function (v) { return '<li>' + esc(v) + '</li>'; }).join('') + '</ul>';
          html += '<tr><td class="sq-prop-key">' + esc(displayKey) + '</td><td class="sq-prop-val">' + displayVal + '</td></tr>';
        }
        html += '</table>';
      }
    }

    // IRI (compact, wrapped) — suppress skolem IRIs
    if (iri && iri.indexOf('//2e.aonprd.com/instances/') === -1) {
      html += '<div class="sq-detail-iri">' + esc(iri) + '</div>';
    }
    // Connection count
    ...
    // Neighbor list (first 20), filtering out skolem IRI nodes
    if (neighborCnt > 0) {
      var nlHtml = '<div class="sq-neighbor-count">Neighbors (' + neighborCnt + ')</div>';
      var shown  = 0;
      var skipped = 0;
      for (var ni = 0; ni < neighbors.length && shown < 20; ni++) {
        var nIdx   = neighbors[ni];
        var nIri   = nodeIris[nIdx] || '';
        if (nIri && nIri.indexOf('//2e.aonprd.com/instances/') !== -1) {
          skipped++;
          continue;
        }
      ...
      var remaining = neighborCnt - shown - skipped;
      if (remaining > 0) nlHtml += '<div class="sq-neighbor-item" style="color:#707070">… and ' + remaining + ' more</div>';
  ...
  function startKeepAlive() {
    ...
    setInterval(function () {
      // Skip re-energizing while paused, otherwise the 2s tick would
      // immediately undo cosmos.pause() (which only clears isSimulationRunning).
      if (cosmos !== null && !simPaused) cosmos.start(0.1);
    }, 2000);
  }
  ...
  function applyPhysics() {
    ...
    // cosmos.gl re-energizes the layout via start(alpha). Skip while paused so a
    // slider tweak doesn't silently resume — the new config takes effect on Resume.
    if (!simPaused) cosmos.start(0.2);
  }
  ...
  // ── Feature: Physics Pause/Resume toggle ─────────────────────────────────
  var pauseBtn  = document.getElementById('sq-physics-pause');
  if (pauseBtn) {
    pauseBtn.addEventListener('click', function () {
      ...
      if (simPaused) {
        ...
        pauseBtn.classList.remove('sq-paused');
      } else {
        simPaused = true;
        cosmos.pause();
        pauseBtn.textContent = '▶ Resume';
        pauseBtn.classList.add('sq-paused');
      }
    });
  }
  ...
  fetch(manifestUrl)
    ...
    .then(function (data) {
      ...
      frameMeta = data.frames.map(function (f) {
        return { label: f.label, color: f.color, nodeCount: f.nodeCount, nodeBase: f.nodeBase, dataFile: f.dataFile || null };
      });
  ...
  window.__sq = {
    ...
    nodeDataCache:  function () { return nodeDataCache; },
