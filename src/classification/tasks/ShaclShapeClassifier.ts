...
// ──────────────────────────────────────────────────────────────────────────────
// `classify:shacl-shape` self-registering plugin (silo migration, task #18)
// ──────────────────────────────────────────────────────────────────────────────

const pluginLog = Logger.forComponent('classify:shacl-shape');

/** Pipeline name shared by the lifecycle hook and the per-record task. */
export const SHACL_SHAPE_PLUGIN_NAME = 'classify:shacl-shape' as const;

/** Default proposal priority when `config.priority` is omitted. */
const SHACL_SHAPE_DEFAULT_PRIORITY = 45;
...
TaskRegistry.registerHook(SHACL_SHAPE_PLUGIN_NAME, 'onRunStart', async (ctx) => {
  const rawConfig = (ctx.config as Record<string, unknown>)['shaclShape'];
  if (rawConfig === undefined) {
    pluginLog.debug('onRunStart', 'no shaclShape config; plugin idle for this run');
    return;
  }
  ...
  const priority = config.priority ?? SHACL_SHAPE_DEFAULT_PRIORITY;
  ...
  if (config.shapesFrom !== 'ontology') {
    ...
    pluginLog.debug('onRunStart', 'Loaded shape file', { absPath, quadCount: parsed.quads.length });
  ...
  pluginLog.debug('onRunStart', 'classify:shacl-shape primed', {
    target:     ctx.target,
    shapesFrom: config.shapesFrom,
    priority,
  });
...
const shaclShapeTask: TaskFnInterface<PipelineStateInterface> = async (
  ...
  if (allShapeQuads.length === 0) {
    pluginLog.debug('execute', 'Shape graph is empty, no-op', { targetId: state.targetId });
    await next();
    return;
  }
  ...
  if (shapes.length === 0) {
    pluginLog.debug('execute', 'No NodeShapes found, no-op', { targetId: state.targetId });
    await next();
    return;
  }
  ...
  for (const shape of shapes) {
    ...
    try {
      ...
      pluginLog.debug('execute', 'SHACL validation threw for shape', {
        targetId: state.targetId,
        shapeIri: shape.shapeIri,
        error:    err instanceof Error ? err.message : String(err),
      });
    ...
    if (report.conforms) {
      proposals.push({
        source:     SHACL_SHAPE_PLUGIN_NAME,
        className,
...
TaskRegistry.register(SHACL_SHAPE_PLUGIN_NAME, shaclShapeTask, { proposesClass: true });
 ...
 * construction logic for the silo plugin form. Static-only by design — no
 * instances, no state. Mirrors the private-method surface that
 * {@link ShaclShapeClassifier} carries today, but scoped to this plugin
 * surface.
 *
 * @internal
 */
