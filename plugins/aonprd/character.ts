...
interface BaseShape {
  url:             string;
  /** Numeric AON entity ID from the URL query string. */
...
function baseFrom(c: CommonExtraction, $: CheerioAPI): BaseShape {
  return {
    url:             c.url,
    entity_id:       extractEntityId(c.url),
    name:            c.title.name,
...
export interface BackgroundOutput extends BaseShape {
  ...
  flavor_text:            string;
  /**
   * Related source books from the `<b>Related Sources</b>` field.
   * These link to Sources.aspx entries that feature this background.
   */
...
export function extractBackground(c: CommonExtraction, _$: CheerioAPI, _span: CheerioNode): BackgroundOutput {
  ...
  // Related Sources: Sources.aspx links in the "Related Sources" field.
  const related_sources: Array<{ name: string; source_id: number | null }> = [];
