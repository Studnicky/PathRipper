...
export function extractAnimalCompanionCombat(c: CommonExtraction): AnimalCompanionCombatSlice {
  const strikes = [];
  for (let i = 0; i < c.fields.length; i++) {
    ...
    const peek = c.fields[i + 1];
    if (peek !== undefined && peek.label.toLowerCase() === 'damage') {
      damage = peek.value_text;
    }
    strikes.push(parseStrikeValue(lower === 'melee' ? 'melee' : 'ranged', e.value_html, damage));
  }
  const support  = findField(c.fields, 'Support Benefit');
  const maneuver = findField(c.fields, 'Advanced Maneuver');
