...
import type { CommonExtraction, CheerioNode, ActionCost } from '../../common.js';
import {
  htmlToText,
  asInt,
  splitTopLevel,
...
export function parsePrice(raw: string | null): PriceParts {
  ...
  let goldPieces: number | null = null, silverPieces: number | null = null, copperPieces: number | null = null;
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (unit === 'gp') goldPieces = num;
    else if (unit === 'sp') silverPieces = num;
    else if (unit === 'cp') copperPieces = num;
  }
  return { gp: goldPieces, sp: silverPieces, cp: copperPieces, raw: text };
}
...
export function parseBulk(raw: string | null): 'L' | number | null {
  ...
  if (/^L$/i.test(trimmed)) return 'L';
  const num = asInt(trimmed);
  return num;
}
...
export function parseDamage(raw: string | null): DamageParts | null {
  ...
  const match = /^(\d+d\d+(?:\s*[+-]\s*\d+)?)\s*([BPS])?\s*(.*)$/i.exec(trimmed);
...
export function parseActivation(valueHtml: string): Activation {
  ...
  while ((glyphMatch = glyphRe.exec(valueHtml)) !== null) {
    ...
    const glyphLabel = ACTION_GLYPH_RE.exec(inner);
    if (glyphLabel === null) continue;
    const cost = ACTION_LABEL_TO_COST.get((glyphLabel[1] ?? '').toLowerCase());
    if (cost !== undefined) costs.push(cost);
  }
  let action_cost: ActionCost | null = null;
  if (costs.length === 1) action_cost = costs[0] ?? null;
  ...
  if (compMatch !== null) {
    for (const part of splitTopLevel(compMatch[1] ?? '', ',')) {
      const lcPart = part.toLowerCase().replace(/\s+/g, ' ').trim();
      if (lcPart !== '') components.push(lcPart);
    }
  }
...
export function parseVariants(common: CommonExtraction): EquipmentVariant[] {
